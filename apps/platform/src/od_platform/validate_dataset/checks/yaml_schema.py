...
def validate_yaml_schema(ctx: CheckContext) -> CheckResult:
    ...
    if nc is None or nc <= 0:
        problems.append(f"nc字段不存在或nc字段的值不是正整数：{snap.yaml_data.get('nc')}")

    names = snap.class_names
    if not names:
        raw = snap.yaml_data.get("names")
        problems.append(f"names缺失或者是不合法的字典或列表：{type(raw).__name__}")

    if nc is not None and nc > 0 and names and (len(names) != nc):
        problems.append(f"nc 和names长度不一致，nc:{nc},names:{names}")

    if problems:
        return CheckResult(
            name = "yaml_schema",
            severity = CheckSeverity.ERROR,
            summary = f"yaml_schema字典不一致：共有{len(problems)}个问题",
            details = {
                "reason": f"field_incosistency",
                "problem": problems,
                "nc" : nc,
    ...
    return CheckResult(
        name="yaml_schema",
        severity=CheckSeverity.PASS,  # 或 CheckSeverity.INFO（根据你的定义）
        summary="yaml_schema 检查通过，所有字段均符合规范",
        details={
            "nc": nc,
            "names_count": len(names) if names else 0,
